...
- [業務記録テンプレ（PDF）をダウンロード](/blog/gyoumu-no-kiroku-template/gyoumu-no-kiroku-template.pdf) — 1枚で1業務分。印刷してすぐ手書きで始められます
- [業務記録テンプレ（Excel編集用）をダウンロード](/blog/gyoumu-no-kiroku-template/gyoumu-no-kiroku-template.xlsx) — 空欄の「業務の記録」シートと「記入例」シートの2枚入り

全列の意味は第3章、記入手順と記入済みサンプルは第4章・第5章で解説しています。

業務の記録は「毎業務ごとに書き続けられるか」がカギです。[delilog アプリ](https://apps.apple.com/jp/app/delilog/id6753698337)なら開始・終了・休憩をタップで記録するだけで、同じ様式のPDFが自動で出来上がります（14日間無料トライアル）。
